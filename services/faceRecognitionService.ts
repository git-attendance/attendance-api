...
export class FaceRecognitionService {
   ...
   * @param filename - Original filename of the uploaded photo
   */
  async verifyPerson(photoBuffer: Buffer, store: string = "1", filename: string = "photo.jpg") {
    try {
      const form = new FormData();
      ...
      if (match.probability < 0.8) {
        const error = new Error(
          `Face verification confidence too low: ${match.probability}`
        ) as any;
        error.statusCode = 400;
  ...
  async getPersonDetails(personId: string) {
    try {
      ...
      if (error.response) {
        const err = new Error(
          error.response.data.message || "Failed to retrieve person details"
        ) as any;
        err.statusCode = error.response.status;
